...
/* Desafio Batalha Naval - MateCheck
   Este código implementa um jogo de Batalha Naval com navios posicionados em um tabuleiro 10x10.
   O jogador pode posicionar navios horizontalmente, verticalmente ou diagonalmente, e usar habilidades
   especiais para atacar áreas do tabuleiro. As habilidades disponíveis são Cone, Cruz e Octaedro.
   O tabuleiro é exibido após cada ação, permitindo ao jogador visualizar o estado atual do jogo.

   Funcionalidades:
   - Posicionar navios em diferentes orientações
   - Usar habilidades para atacar áreas do tabuleiro
   - Exibir o tabuleiro atualizado após cada ação

   @author Daniel dos Santos Marfim (https://github.com/Dayil)
   @date 03/05/2025
...
// Constantes do jogo e tamanho das matrizes
#define AGUA 0
#define QT_NAVIO 3 // Quantidade de navios
#define HABILIDADE 5 // Quantidade de habilidade
// tamanhos
...
int lerInteiro(const char* mensagem) {
    ...
    while (1) {
        ...
        if (scanf("%d", &valor) == 1) {
            ...
            // Limpa o buffer evitando loop infinito
            scanf("%s", buffer);
        }
    }
}
    ...
    do {
        ...
        switch (opcao) {
            case 1: // Posicionar navio
                ...
                switch (tipo_navio) {
                    case 1: sucesso = posicionarNavio(tabuleiro, linha, coluna, true); break;
                    case 2: sucesso = posicionarNavio(tabuleiro, linha, coluna, false); break;
            ...
            case 2: // Usar habilidade
                ...
                switch (tipo_habilidade) {
                    ...
                    case 3: aplicarHabilidade(tabuleiro, octaedro, linha, coluna); break;
...
bool posicionarNavio(int tabuleiro[TAM_TABULEIRO][TAM_TABULEIRO], int linha, int coluna, bool horizontal) {
    ...
    for (int i = 0; i < TAM_NAVIO; i++) {
        if (horizontal) {
            tabuleiro[linha][coluna + i] = QT_NAVIO;
        } else {
            tabuleiro[linha + i][coluna] = QT_NAVIO;
        }
    }
...
for (int i = 0; i < TAM_NAVIO; i++) {
    ...
    tabuleiro[linha_atual][coluna_atual] = QT_NAVIO;
    }
    return true;
}
